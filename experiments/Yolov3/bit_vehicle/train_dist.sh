...
EXP_NAME=${1-debug}
CONFIG_FILE=${2-${ROOT}/configs/yolo/yolov3_d53_mstrain-416_273e_ua-detrac.py}
GPUS=${3-'0,1,2,3'}
# get gpu-nums
gpu_ids=(${GPUS//,/ })
ngpus=${#gpu_ids[@]}
...
python -m torch.distributed.launch --nproc_per_node=${ngpus} --master_port=29522 \
${ROOT}/tools/train.py \
    ${CONFIG_FILE} \
    --work-dir=${OUTPUT_DIR} \
    --launcher pytorch \
    --resume-from output/baseline_yolov3_d53_uadetrac_4gpus/latest.pth \
